...
class ViewController: UIViewController {
    ...
    /// Adds a node a user's touch location represent by point
    /// - Parameters:
    ...
    func  addNode(_ node: SCNNode, to parentNode: SCNNode) {
        ...
        // Chech that the object is nottoo close to the previous one
        ...
        // Remeber object placed for undo
    ...
    func reloadConfiguration(reset: Bool = false) {
        ...
        let option: ARSession.RunOptions = reset ? .removeExistingAnchors : []
        
        // Reload configuration
        configuration.detectionImages = ARReferenceImage.referenceImages(inGroupNamed: "AR Resources", bundle: nil)
        configuration.planeDetection = .horizontal
        sceneView.session.run(configuration, options: option)
    }
